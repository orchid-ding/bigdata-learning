...
import top.kfly.pojo.{Order, OrderObj}
...
class HBaseSinkFunction extends RichSinkFunction[OrderObj]{
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    hbTable = connection.getTable(TableName.valueOf("flink:kfly_orders"))
  ...
  override def invoke(orderObj: OrderObj, context: SinkFunction.Context[_]): Unit = {
    ...
    if (database.equalsIgnoreCase("flink_house") && table.equalsIgnoreCase("kfly_orders")) {
